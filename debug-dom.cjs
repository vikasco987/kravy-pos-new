...
await page.on('console', msg => {
  if (msg.type() === 'error') console.error('CONSOLE ERROR:', msg.text());
    ...
    try {
      ...
      console.log(await page.content()); await browser.close(); process.exit(0);
      continue;
    }

    console.log("Waiting for 'Manage your products'...");
    try {
      await page.waitForFunction(() => {
        return document.body.innerText.includes('Manage your products');
      }, { timeout: 15000 });
    } catch (e) {
    ...
    for (let i = 0; i < term.length; i++) {
      ...
      let tStart = 0;
      if (isLast) {
        prevCount = await page.evaluate(() => document.querySelectorAll('h5').length);
      ...
      await page.fill('input[placeholder*="Search"]', currentStr);
      ...
      if (!isLast) {
        ...
        const afterCount = await page.evaluate(() => document.querySelectorAll('h5').length);
        const matchingText = await page.evaluate((q) => {
          const card = Array.from(document.querySelectorAll('h5')).find(c => c.textContent.includes(q));
