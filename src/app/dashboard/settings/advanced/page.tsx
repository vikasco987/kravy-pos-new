        ...
        <div className="bg-[var(--kravy-surface)] border border-[var(--kravy-border)] rounded-[32px] p-8 shadow-xl flex flex-col md:flex-row items-center gap-8 group">
          ...
          <div className="flex-1 space-y-2">
            <h3 className="text-xl font-black text-[var(--kravy-text-primary)] flex items-center gap-2">
              ...
              <span className="text-[10px] bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-black uppercase tracking-widest">New</span>
            </h3>
            <p className="text-sm text-[var(--kravy-text-muted)] font-medium max-w-md leading-relaxed">
              Enable the full Hotel Property Management System. Includes Room Grid, Stays, Checkout workflow, and Financials.
            </p>
          </div>
          <button
            onClick={() => handleSave({ enableHotelManagement: !profile.enableHotelManagement })}
            disabled={saving}
            className={`px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${
              profile.enableHotelManagement 
                ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' 
                : 'bg-slate-100 text-slate-400 dark:bg-slate-800'
            }`}
          >
            {profile.enableHotelManagement ? 'Enabled' : 'Disabled'}
          </button>
        </div>
